...
class EventCard(Card):
    ...
    def sort_players(self, players, resource):
        def sort_key(player):
            ...
            return (-amount, self.match.players.index(player))
...
class EntenteCordialeGeneralStrike(Age4EventCard):
    ...
    def happen_a(self):
        def military_deploys(player):
            possible_deploys = []
            for (slot, card) in player.filled_slot_names_cards():
                if card.is_military() and not (card.max_workers and card.deployed_workers == len(card.worker_points)):
                ...
                if player.grown_workers:
                    for i in range(2):
                        ...
                        if possible_deploys:
                            ...
                            self.match.log(f'{player} deploys to "{deploy.card}" for free.')
                            player.deploy_for_free(deploy.card)
                        else:
                            self.match.log(f'{player} has no room on militaries.')
                            break
                    if player.need_confirmation:
                        self.match.get_move(player, 'Confirm?', ('Confirm',))
                else:
                    self.match.log(f'{player} has no workers.')
            else:
