...
class Player:
    ...
    def buy_actions(self, free=False):
        possible_buys = []
    ...
    def can_deploy_to_card(self, card, free=False):
        if card.max_workers and card.deployed_workers == len(card.worker_points):
            return False
        if free:
            return True
    ...
    def deploy_actions(self, free=False):
        ...
        for (slot, card) in self.filled_slot_names_cards():
            if card.is_building_military() and self.can_deploy_to_card(card, free=free):
    ...
    def resign_action(self, action):
        ...
        self.match.get_move(self, 'Confirm?', ('Confirm',))
        self.match.log(f'{self} resigns.')
        other_player = [player for player in self.match.players if player is not self][0]
        self.match.log(f'{other_player} wins!')
    ...
    def take_action(self, action):
        return {
            ...
            ActionType.RESIGN: self.resign_action
    ...
    def take_one_action(self):
        ...
        if possible_actions:
            ...
            if len(self.match.players) == 2:
                possible_actions.append(ResignAction())
            ...
            if board_card in cards_to_remove:
                ...
                for card in reversed(card_stack):
                    ...
                    if card.is_building_military():
                        ...
                        if workers:
                            self.match.log(f'{self} undeploys {workers} [Worker{s_if_not_1(workers)}] from "{card}".')
