...
class PorcelainTower(Age2Wonder):
    ...
    def ready(self):
        ...
        self.register_for_event('cover card', self.is_self, self.true)
        ...
        if answer == 'Yes':
            if player.workers == 0:
                ...
                if len(possible_undeploys) == 1:
                    ...
                    undeploy = self.match.get_move(self, 'Undeploy worker from where?', possible_undeploys)
...
class ZhuXi(Age2Advisor):
    ...
    def buy(self):
        self.register_for_event('cover card', self.not_covered, self.cover)
    ...
    def not_covered(self, player, **kwargs):
        return player is self.owner and kwargs['card'] is self and self.covered_by is None
...
class PeterTheGreat(Age3Advisor):
    ...
    def produce(self, projected=False):
        ...
        if projected:
            return production
        if self.match.war is not None and self.owner.resources[Resource.MILITARY] > self.match.war_value:
...
class VictoriaFalls(Age4NaturalWonder):
    ...
    def discovered(self):
        drawn_cards = self.match.progress_cards[4][:20]
        del self.match.progress_cards[4][:20]
